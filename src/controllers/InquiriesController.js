...
module.exports = {
  async index(request, response) {
    ...
    if(searchdata!="" && searchdata){
      ...
      inquiries = await connection("inquiries").where('inquiries.num_inq', 'like',`%${searchdata|| ''}%`).orWhere('involved.name_involv','like',`%${searchdata || ''}%`)
      .limit(limit)
  ...
  async create(request, response) {
    ...
    const inquiry = await connection("inquiries")
      ...
      .orWhere("num_in_justice_inq", num_in_justice_inq)
      .select("cod_inq")
      .first();
  ...
  async update(request, response) {
    ...
    if (
      ...
      return response
        .status(400)
        .json({ response: "Preencha todos os camposs" });
    }
    ...
    const checkInquiryWithnumbers_numinq = await connection("inquiries")
      ...
      .select("cod_inq")
      .first();
      const checkInquiryWithnumbers_numinqjust = await connection("inquiries")
      .where("num_in_justice_inq", num_in_justice_inq)
      .select("cod_inq")
      .first();
      
    if ((checkInquiryWithnumbers_numinq && checkInquiryWithnumbers_numinq.cod_inq!=id) || (checkInquiryWithnumbers_numinqjust && checkInquiryWithnumbers_numinqjust.cod_inq!=id)) {return response.status(401).json({
